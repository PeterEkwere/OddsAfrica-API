...
def load_json(file_path, betsite_file):
...
def process_betsite_files(folder_path):
    ...
    for i in range(len(betsite_files) - 1):
        for j in range(i + 1, len(betsite_files)):
            ...
            betsite1_data = load_json(file1_path, betsite_files[i])
            betsite2_data = load_json(file2_path, betsite_files[j])
...
def process_games():
    """ This function process all games data from all sports and saves them in their respective directory
    """
    ...
    for sport_folder in os.listdir(sports_folder):
        try:
            ...
            if os.path.isdir(sport_folder_path):
                filename = f"{sport_folder}_found_games.json"
                filepath = f"engine/storage_engine/processed_data/{sport_folder}/"
                try:
                ...
                found_games = process_betsite_files(sport_folder_path)
